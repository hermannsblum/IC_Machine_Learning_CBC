...
Furthermore we considered to use \emph{regularization}, but this would include another parameter to optimise. Because the optimisation of our parameters already took approximately two days, we could not spend triple/quadrupel/... of time. \\
However, theoretically also other techniques, for example \emph{dropout}, are possible. Data Augmentation by using the noisy dataset additionally to the clean dataset would be an option. All these approaches are described in the lecture slides, which is why we won't copy it here.\\

\subsubsection{Question 4}
\textbf{Disadvantages} \\
First of all this leads to a necessary implementation of a decision function, which classifies on the basis of the outputs of the six networks. As in the assignment before, there is no obvious best solution for this problem and therefore the choice of decision function is another parameter to optimise.\\
